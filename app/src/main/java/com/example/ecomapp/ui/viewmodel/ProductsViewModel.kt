...
//                    else -> suggestProducts.postValue(response.body())
    ...
    fun fetchProductsByPriceAndType(price: Int, typeId: Int) {
        viewModelScope.launch(Dispatchers.IO) {
            val response = productRepository.fetchProductsByPriceAndType(price, typeId)
            if (response.isSuccessful) {
    ...
    fun fetchProductsFilterByCategoriesAndPrice(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (response.isSuccessful) {
                ...
                Log.i(
                    "Error",
                    "Error: code = ${response.code()} and message = ${
