...
class DisplayProductFragment : Fragment() {
    ...
    private var currentCategoryIds = ArrayList<Int>()

    private val maxPrice: Int = 2000000
    private var currentPrice: Int = maxPrice
    ...
    override fun onCreateView(
        ...
        binding.apply {
            ...
            when (type) {
                ...
                "3" -> {
                    tvTopTitle.text = "Tìm kiếm: ${keyword}"
                    tvTopTitle.textSize = 15f
                    btnFilter.visibility = GONE
                }
    ...
    private fun fetchDisplayProductsApplyFilter(
        type: Int,
        listCategoryId: List<Int>,
        listCount: Int,
        price: Int
    ) {
        val productsViewModel = ViewModelProvider(this)[ProductsViewModel::class.java]
        if (type == 0) {
            productsViewModel.fetchProductsFilterByCategoriesAndPrice(
                listCategoryId,
                listCount,
                price
            )
        } else if (type == 1) {
            productsViewModel.fetchProductsFilterByCategoriesAndPriceAndType(
                type,
                listCategoryId,
                listCount,
                price
...
//        }
    ...
    private fun fetchDisplayProductsByPriceAndType(price: Int) {
        val productsViewModel = ViewModelProvider(this)[ProductsViewModel::class.java]
        productsViewModel.fetchProductsByPriceAndType(price, type.toInt())
    }
    ...
    private fun createDialogFilter() {
        ...
        val categoryAdapter = CategoryRecyclerViewAdapter(listCategory, currentCategoryIds)
        ...
        dialog.findViewById<Button>(R.id.btnApply).setOnClickListener {
            if (categoryAdapter.listCategoryId.isEmpty() && currentPrice == maxPrice) {
                addDisplayProducts(allProducts)
            } else if (categoryAdapter.listCategoryId.isEmpty() && currentPrice < maxPrice) {
                fetchDisplayProductsByPriceAndType(currentPrice)
            } else {
                fetchDisplayProductsApplyFilter(
                    type.toInt(),
                    categoryAdapter.listCategoryId,
                    categoryAdapter.listCategoryId.size,
                    currentPrice
                )
                currentCategoryIds = categoryAdapter.listCategoryId
            }
            dialog.dismiss()
        }
    ...
    private fun initializeSeekbar(startPrice: TextView, endPrice: TextView) {
        ...
        seekBar.max = 100
        seekBar.progress = currentPrice / 20000
        ...
        seekBar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            ...
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                ...
                startPrice.text = formatPrice(currentPrice)
                endPrice.text = formatPrice(maxPrice)
